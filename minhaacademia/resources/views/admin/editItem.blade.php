...
<div class="section main">
    <div class="container u-full-width">
        <h2>Editar Item - Questão {{ $item->question()->order + 1 }} - <a href="{{ route('activityEdit', ['activity' => $item->question()->activity]) }}">{{ $item->question()->activity()->title }}</a></h2>
        @if ($errors->any())
        ...
        <form action="{{ route('itemUpdate', ['item' => $item->id]) }}" method="post">
            @csrf
            @method('PUT')
            <div class="u-full-width text-left">
                <label for="content" class="u-pull-left">Conteúdo:</label>
                <textarea class="u-full-width textarea-small" name="content" id="content">{{ old('content') ? old('content') : $item->content }}</textarea>
                <p class="information">
            ...
            <div class="u-full-width text-left">
                ...
                <select name="order" id="order">
                    @foreach($item->question()->items as $it)
                        <option value="{{ $it->order }}" {{ $it->order == $item->order ? 'selected' : ''}}>{{ $it->orderStr() }}</option>
                    @endforeach
                </select>
                <br>
            </div>
        ...
        <div class="u-full-width text-left">
            ...
            <form action="{{ route('imageStore') }}" method="post" enctype="multipart/form-data">
                ...
                <p class="information">
                    <small>Após anexar uma imagem, use no conteúdo do item o código [img]URL[/img] para adicioná-la.</small>
                </p>
            </form>
        </div>
    </div>
